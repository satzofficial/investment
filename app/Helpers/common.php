...
function encryptIt($string)
{
    ...
    $secret_key = 'bRuD5WYw5wd0rdSATZHR9yLlM6wt2vteuiniQBqE70nAuhU=';
...
{
    $query = DB::table('users')->select('username')->where('id', $id)->first();
    return $query->username;
}

function getUserImage($id)
{
    $query = DB::table('users')->select('image')->where('id', $id)->first();
...
function customerImage($id)
{
    if (!$id) {
        return dummyuserImg();
    }
    $select = 'profile_image';
    $query = DB::table('customers')->select($select)->where('id', $id)->first();
...
{
    $cond = array('user_id' => $user_id, 'polls_id' => $polls_id);
    $query = DB::table('votepolls')->where($cond)->first();
    if ($query)
        return $query->status;
    else
        return false;
...
//     $cond = array('polls_id'=>$polls_id, 'status'=>$opt);
//     $query = DB::table('votepolls')->where($cond)->count();
//     return $query;    
// }

// function getAllPolls($polls_id) {
//     $cond = array('polls_id'=>$polls_id);
//     $query = DB::table('votepolls')->where($cond)->count();
//     return $query;    
// }
...
if (!function_exists('isStrExistOrNot')) {

    function isStrExistOrNot($array, $commonPrefix)
    {
        if (!$array && !$commonPrefix) {
            return false;
        }
        foreach ($array as $string) {
            if (Str::startsWith($commonPrefix, $string)) {
                return true; // If any string starts with the common prefix, return true
            }
        }
        return false;
    }
